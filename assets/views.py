...
@csrf_exempt
def tagsForm(request):
    # curl -H "Content-Type:application/json" -X POST
    # -d 'asset_data:{"name":"service_name","sn":1234111,"env":"p07"}'
    # http://127.0.0.1:8000/assets/uploadtags
    if request.method == 'POST':
        try:
            ...
            asset = models.Asset.objects.create(asset_type="server",
                                                name=name,
                                                sn=sn,
                                                environment=env,
                                                # approved_by="admin"
            ...
            asset.save()
            return HttpResponse("upload success!", status=200, content_type="text/plain")
        except Exception as e:
            return HttpResponse(e,status=500,content_type="text/plain")
    else:
        return HttpResponse('Please use POST request!',status=500,content_type="text/plain")
